...
public class VideoActivity extends Activity implements AnyChatBaseEvent,
		OnClickListener, AnyChatTextMsgEvent {
	...
	private void CheckVideoStatus() {
		...
		// if (!bSelfVideoOpened) {
		// if (anychat.GetCameraState(-1) == 2 && anychat.GetUserVideoWidth(-1)
		// != 0) {
	...
	private void adjuestVideoSize(int width, int height) {
		...
		// FrameLayout.LayoutParams layoutParamSelf=new
		// FrameLayout.LayoutParams(dwLocalVideoWidth, dwLocalVideoHeight);
		// myView.setLayoutParams(layoutParamSelf);
		LinearLayout.LayoutParams layoutPramOther = new LinearLayout.LayoutParams(
				dwLocalVideoWidth, dwLocalVideoHeight);
	...
	private void InitialLayout() {
		...
		// if(AnyChatCoreSDK.GetSDKOptionInt(AnyChatDefine.BRAC_SO_LOCALVIDEO_CAPDRIVER)
		...
		btn.setText("send info");
		btn.setOnClickListener(new OnClickListener() {

			@Override
			public void onClick(View arg0) {
				// TODO Auto-generated method stub
				anychat.SendTextMessage(-1, 0, "1111");
			}
		});
		otherView.getViewTreeObserver().addOnGlobalLayoutListener(
				new OnGlobalLayoutListener() {
					...
					public void onGlobalLayout() {
						// TODO Auto-generated method stub
						if (!bVideoAreaLoaded) {
							adjuestVideoSize(otherView.getWidth(),
									otherView.getHeight());
							bVideoAreaLoaded = true;
						}
					}
				});
		// mCameraSwitchImage = (ImageView)
		// findViewById(R.id.image_switch_camera);
	...
	public void OnAnyChatTextMessage(int dwFromUserid, int dwToUserid,
		...
		if(dwFromUserid == userID)
			Toast.makeText(this, "teach you sth.", 0).show();
		else
			Toast.makeText(this, "SB TALKED.", 0).show();
